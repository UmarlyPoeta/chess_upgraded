...
def pawn_valid_move(start_pos: tuple, end_pos: tuple, board: ChessBoard) -> bool:
    ...
    # Correct directions based on actual board setup:
    # White pawns start at row 6 and move toward row 0 (direction = -1)
    # Black pawns start at row 1 and move toward row 7 (direction = +1)
    direction = -1 if board.board[start_pos[0]][start_pos[1]].color == "white" else 1
    start_row = 6 if board.board[start_pos[0]][start_pos[1]].color == "white" else 1
    ...
    if end_pos[1] == start_pos[1]:  # Moving forward
        if end_pos[0] - start_pos[0] == direction:
            ...
            if board.board[start_pos[0] + direction][start_pos[1]] != "." or board.board[end_pos[0]][end_pos[1]] != ".":
                return False
            return True
